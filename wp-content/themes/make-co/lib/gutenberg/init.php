...
add_action( 'wp_enqueue_scripts', 'make_co_enqueue_gutenberg_frontend_styles' );
/**
 * Enqueues Gutenberg front-end styles.
 *
 * @since 2.7.0
 */
function make_co_enqueue_gutenberg_frontend_styles() {

	$child_theme_slug = defined( 'CHILD_THEME_NAME' ) && CHILD_THEME_NAME ? sanitize_title_with_dashes( CHILD_THEME_NAME ) : 'make-co';
...
add_action( 'enqueue_block_editor_assets', 'make_co_block_editor_styles' );
/**
 * Enqueues Gutenberg admin editor fonts and styles.
 *
 * @since 2.7.0
 */
function make_co_block_editor_styles() {

	wp_enqueue_style(
		'make-co-gutenberg-fonts',
...
add_filter( 'body_class', 'make_co_blocks_body_classes' );
/**
 ...
 */
function make_co_blocks_body_classes( $classes ) {
...
add_action( 'after_setup_theme', 'make_co_content_width', 0 );
/**
 * Set content width to match the “wide” Gutenberg block width.
 */
function make_co_content_width() {

	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- See https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/924
	$GLOBALS['content_width'] = apply_filters( 'make_co_content_width', 1062 );
