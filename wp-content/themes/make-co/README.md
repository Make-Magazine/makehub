...
GitHub project link: https://github.com/studiopress/make-co/.
...
The version of [Genesis Sample on GitHub](https://github.com/studiopress/make-co/) includes tooling to check code against WordPress standards. To use it:
...
- `npm run makepot` to regenerate the `languages/make-co.pot` file.
- `npm run lint:css` to generate a report of style violations for `style.css`.
- `npm run zip` to create a make-co.zip of the current branch. Excludes files marked export-ignore in `.gitattributes`.
...
4. Type `npm run zip` to create `make-co.zip`. Files marked export-ignore in `.gitattributes` are excluded from the zip.

The `zip` command is an alias for `git archive -o make-co.zip --prefix=make-co/ HEAD`.
