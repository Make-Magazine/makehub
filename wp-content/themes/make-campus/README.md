...
GitHub project link: https://github.com/studiopress/make-campus/.


## Installation Instructions

1. Upload the Make - Co Theme folder via FTP to your wp-content/themes/ directory. (The Genesis parent theme needs to be in the wp-content/themes/ directory as well.)
2. Go to your WordPress dashboard and select Appearance.
3. Activate the Make - Co Theme.
...
The version of [Genesis Sample on GitHub](https://github.com/studiopress/make-campus/) includes tooling to check code against WordPress standards. To use it:
...
- `npm run makepot` to regenerate the `languages/make-campus.pot` file.
- `npm run lint:css` to generate a report of style violations for `style.css`.
- `npm run zip` to create a make-campus.zip of the current branch. Excludes files marked export-ignore in `.gitattributes`.
...
4. Type `npm run zip` to create `make-campus.zip`. Files marked export-ignore in `.gitattributes` are excluded from the zip.

The `zip` command is an alias for `git archive -o make-campus.zip --prefix=make-campus/ HEAD`.
