<div class="ihc-stuffbox">
  <h3><?php _e('Order Details', 'ihc');?></h3>
  <div class="inside">
    <?php
    $orderId = isset( $_GET['order_id'] ) ? $_GET['order_id'] : 0;
    $orderObject = new \Indeed\Ihc\Db\Orders();
    $orderData = $orderObject->setId( $orderId )
                             ->fetch()
                             ->get();
    $orderMetaObject = new \Indeed\Ihc\Db\OrderMeta();
    $orderMeta = $orderMetaObject->getAllByOrderId( $orderId );

    if ( !$orderId || !$orderData ):?>
        <h5><?php _e( "No order details available!", 'ihc' );?></h5>
    <?php else :?>

        <div><?php echo __( 'Membership: ', 'ihc' ) , \Indeed\Ihc\Db\Memberships::getMembershipLabel( $orderData->lid );?></div>
        <h5><?php _e( 'Order Base Data:', 'ihc' );?></h5>
        <?php foreach ( $orderData as $key => $value ):?>
            <div>
                <?php echo $key . ' - ' . $value;?>
            </div>
        <?php endforeach;?>
        <h5><?php _e( 'Order Meta:', 'ihc' );?></h5>
        <?php foreach ( $orderMeta as $metaKey => $metaValue ):?>
            <div>
                <?php echo $metaKey . ' - ' . $metaValue;?>
            </div>
        <?php endforeach;?>
    <?php endif;?>
  </div>
</div>
