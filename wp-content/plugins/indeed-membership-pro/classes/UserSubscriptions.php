...
class UserSubscriptions
{
    ...
    public static function updateSubscriptionTime( $uid=0, $lid=0, $startTime='', $endTime='', $args=[] )
    {
        ...
        if ( isset( $oldData['expire_time'] ) && strtotime( $oldData['expire_time'] ) < $currentTime && strtotime( $expire ) > $currentTime ){
            /// activate level
            do_action( 'ihc_action_after_subscription_activated', $uid, $lid, $firstTime, $args );
    ...
    public static function getOne( $uid=0, $lid=0 )
    {
        ...
        $query = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}ihc_user_levels
                                    WHERE user_id=%d AND level_id=%d ORDER BY id DESC;",
        $uid, $lid );
    ...
    /*
    public static function getMembersThatEnterInGracePeriod( $id=0, $startTime=0, $endTime=0 )
    {
  			global $wpdb;
        ...
        $gracePeriodValue = $gracePeriod * 24 * 60 * 60;
  			$startTime = $startTime + $gracePeriodValue;
        $endTime = $endTime + $gracePeriodValue;
    ...
    */
    ...
    /**
     * Status can be : 0 - canceled, 1 - active, 2 - deleted, 3 - paused
     * @param int
     * @param int ( Subscription id)
     * @return bool
     */
    ...
    {
        global $wpdb;
        $query = "SELECT id,user_id,level_id,start_time,update_time,expire_time,notification,status
                        FROM {$wpdb->prefix}ihc_user_levels
    										WHERE
                        UNIX_TIMESTAMP(expire_time) <= $lowerThan
    										AND UNIX_TIMESTAMP(expire_time) > $graterThan ";
        if ( $now != '' ){
            $query .= " AND UNIX_TIMESTAMP(expire_time) > $now";
        }
