...
class NotificationLogs
{
    ...
    public static function getMany( $uid=0, $limit=30, $offset=0, $onlyUser=false )
    {
        global $wpdb;
        if ( $uid ){
            $query = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}ihc_notifications_logs WHERE uid=%d ORDER BY id DESC LIMIT %d OFFSET %d;",
                                      $uid, $limit, $offset );
        } else {
            $query = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}ihc_notifications_logs ORDER BY id DESC LIMIT %d OFFSET %d;", $limit, $offset );
        }
    ...
    public static function getCount( $uid=0 )
    {
        ...
        if ( $uid ){
          ...
          $data = $wpdb->get_var( $query );
            ...
            $data = $wpdb->get_var( "SELECT COUNT(*) as c FROM {$wpdb->prefix}ihc_notifications_logs;" );
        }
        return $data;
    }
