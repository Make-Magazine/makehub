...
class RegistrationEvents
{
    ...
    public function checkUsername( $errorMessages=[], $postData=[], $registerFields=[], $uid=0 )
    {
        ...
        if ( !$uid ){
            if ( !isset( $postData['user_login'] ) || !validate_username($postData['user_login'])) {
    ...
    public function securityCheckUsername( $errorMessages=[], $postData=[], $registerFields=[], $uid=0 )
    {
        ...
        foreach ( $blacklist as $name ){
            if ( strpos( $postData['user_login'], $name ) !== false ){
                $errorMessages['user_login'] = $theMessage;
                return $errorMessages;
            }
        }
    ...
    public function checkInvitationCode( $errorMessages=[], $postData=[], $registerFields=[], $uid=0 )
    {
        ...
        if ( empty( $postData['uid'] ) && $checkInviteCode && get_option('ihc_invitation_code_enable')){
           if ( empty( $postData['ihc_invitation_code_field'] ) || !\Ihc_Db::invitation_code_check( $postData['ihc_invitation_code_field'] ) ){
               ...
               if (!$errMsg){
                 $errorMessages['ihc_invitation_code_field'] = __('Your Invitation Code is wrong.', 'ihc');
               } else {
     ...
     */
     /*
    public function checkRequired( $errorMessages=[], $postData=[], $registerFields=[], $uid=0 )
    {
        $errorMessage = get_option( 'ihc_register_err_req_fields' );
        $exceptions = isset( $postData['ihc_exceptionsfields'] ) ? explode( ',', $postData['ihc_exceptionsfields'] ) : [];
        ...
        foreach ( $registerFields as $registerField ) {
            ...
            if ( !empty($registerField['req']) && $postData[$name]=='' && !in_array( $name, $exceptions ) ){
              $errorMessages[$name] = $errorMessage;//add the error message
              continue;
            }
            if ( $registerField['type'] == 'unique_value_text' && isset( $postData[$name] ) && \Ihc_Db::doesUserMetaValueExists( $name, $postData[$name], $uid ) ){
                $errorMessages[$name] = __('Error', 'ihc'); // unique value
            }
        }
        return $errorMessages;
    }
    ...
    public function setCustomFields( $customMetaUser=[], $postData=[], $registerFields, $uid=0 )
    {
        if ( empty( $uid ) ){
  			...
  			foreach ( $registerFields as $registerField ){
  					...
  					if ( $registerField['type']=='checkbox' && empty( $postData[$name] ) ){
  							/// empty checkbox
  					...
  					} else if ( $registerField['type']=='single_checkbox' && empty( $postData[$name] )) {
  							...
  							if ( empty( $registerField['native_wp'] ) ){
  								...
  								if ( is_array( $postData[$name] ) ){
  									...
  									$customMetaUser[$name] = filter_var( $postData[$name], FILTER_SANITIZE_STRING);//$_POST[$name];
  								}
  							}
  					}
  			}
