			...
			case 'ihc_dynamic_price':
				if (strcmp($attr['form_type'], 'create')===0 && ihc_is_magic_feat_active('level_dynamic_price') && !empty($attr['lid'])){
					...
					if (!empty($temp_settings['ihc_level_dynamic_price_levels_on'][$lid])){
						...
						$str .= "<input type='range' onChange='ihcDynamicPriceUpdateGlobal();' onBlur='ihcDynamicPriceUpdateGlobal();' min='$min' max='$max' class='{$attr['class']}' step='$step' value='$level_price' name='ihc_dynamic_price' id='ihc_dynamic_price' oninput='setPrice.value = ihc_dynamic_price.value' />$<input name='setPrice' id='setPrice' value='$level_price' min='20' max='9999' type='number'></input><i>(choose your desired amount)</i><script type='text/javascript'>jQuery(document).ready(function(){jQuery('#setPrice').change(function(){jQuery('#iumpfinalglobalp').val(jQuery('#setPrice').val());jQuery('#ihc_dynamic_price').val(jQuery('#setPrice').val());});});</script>";
					}
				}
				break;
		}
