...
class ICal
{
    ...
    /* How many timezones are in this iCal? */
    public /** @type {int} */ $timezone_count = 0;
    
    /* How many timezone offsets are in this iCal? */
    public /** @type {int} */ $timezone_offset_count = array('DAYLIGHT' => 0, 'STANDARD' => 0);
            ...
            foreach ($lines as $line) {
                ...
                foreach ($values as $value) {
                    switch ($line) {
                        ...
                        case 'BEGIN:STANDARD':
                        	if (!is_array($value)) {
                        		$this->timezone_offset_count[$value]++;
                        	}
                        	$component = $value;
                        	break;
    ...
    public function addCalendarComponentWithKeyAndValue($component, $keyword, $value)
    {
        ...
        switch ($component) {
            case 'VTODO':
                ...
                break;
            case 'VTIMEZONE':
            	$this->cal[$component][$this->timezone_count - 1][$keyword] = $value;
            	break;
            case 'STANDARD':
            case 'DAYLIGHT':
            	// EDIT - Marcus - Previous implementation had used $this->timezone_offset_count[$component] - 1 instead of $component, but this didn't allow daylight/standard variations since one overwrite the other
				// Now, you will have both DAYLIGHT and STANDARD by key (if there is one or the other) but theoretically it's possible to have more than 1 STANDARD/DAYLIGHT value which isn't implemented here.
            	$this->cal['VTIMEZONE'][$this->timezone_count - 1]['OFFSETS'][$component][$keyword] = $value;
            	$this->cal['VTIMEZONE'][$this->timezone_count - 1]['OFFSETS'][$component]['TYPE'] = $component;
            	break;
            case 'VEVENT':
