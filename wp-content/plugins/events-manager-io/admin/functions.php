<?php
function emio_input_select_items($array, $saved_value) {
	$output = "";
...
function emio_input_checkbox_items($name, $array, $saved_values, $horizontal = true) {
	$output = "";
	foreach($array as $key => $item) {
		$checked = "";
...
function emio_input_text($title, $name, $value='', $description ='', $ph = false) {
	?>
	<tr valign="top" id='<?php echo esc_attr($name);?>_row'>
	    ...
	    <td>
			<input name="<?php echo esc_attr($name) ?>" value="<?php echo esc_attr($value); ?>" type="text" id="<?php echo esc_attr($name) ?>" class="widefat" <?php if($ph) echo 'placeholder="'.esc_attr($description).'"'; ?> >
...
function emio_input_password($title, $name, $value, $description ='') {
	?>
	<tr valign="top" id='<?php echo esc_attr($name);?>_row'>
		<th scope="row"><label for="<?php echo esc_attr($name); ?>"><?php echo esc_html($title); ?></label></th>
	    <td>
...
function emio_input_textarea($title, $name, $value, $description ='') {
	?>
	<tr valign="top" id='<?php echo esc_attr($name);?>_row'>
		<th scope="row"><label for="<?php echo esc_attr($name); ?>"><?php echo esc_html($title); ?></label></th>
			<td>
...
function emio_input_radio($name, $options, $option = '', $title='') {
		?>
	   	<tr valign="top" id='<?php echo esc_attr($name);?>_row'>
	   		<?php if( !empty($title) ): ?>
...
function emio_input_select($title, $name, $list, $option_value = 0, $description='', $default='') {
	?>
   	<tr valign="top" id='<?php echo esc_attr($name);?>_row'>
   		<th scope="row"><label for="<?php echo esc_attr($name); ?>"><?php echo esc_html($title); ?></label></th>
   		<td>
