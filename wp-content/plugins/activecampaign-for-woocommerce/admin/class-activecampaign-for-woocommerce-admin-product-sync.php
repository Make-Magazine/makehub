 ...
 */
class Activecampaign_For_Woocommerce_Admin_Product_Sync {
	...
	public function get_product_sync_status() {
		try {
			...
			$cron_array      = $this->_get_cron_array();
			...
			if (
				true === $jobs_scheduled ||
				! empty( wp_get_scheduled_event( ACTIVECAMPAIGN_FOR_WOOCOMMERCE_RUN_SYNC_NAME ) ) ||
				get_option( ACTIVECAMPAIGN_FOR_WOOCOMMERCE_SYNC_SCHEDULED_STATUS_NAME )
			) {
				$data = (object) array(
					'status'          => $status,
					'last_sync'       => $last_sync,
