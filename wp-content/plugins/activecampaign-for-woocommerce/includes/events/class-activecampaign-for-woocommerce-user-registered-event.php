...
class Activecampaign_For_Woocommerce_User_Registered_Event implements Triggerable {
		...
		if ( isset( $args[0] ) ) {
			...
			if ( isset( $user_data->user_email ) ) {
				...
				if ( $this->setup_woocommerce_customer() ) {
					...
					$this->logger->error( 'Registered event: Could not save contact to ActiveCampaign.' );
				}
			}
		}
	}
	...
	private function setup_woocommerce_customer() {
		...
		if ( isset( $this->admin ) && isset( $this->admin->get_storage()['connection_id'] ) ) {
			...
			try {
				...
				try {
					...
					$this->logger->debug( 'Registered event: Creating customer in ActiveCampaign. ', [ 'new_customer_data' => \AcVendor\GuzzleHttp\json_encode( $new_customer->serialize_to_array() ) ] );
			...
			$this->logger->error( 'Registered event: Could not retrieve the connection id ' );
