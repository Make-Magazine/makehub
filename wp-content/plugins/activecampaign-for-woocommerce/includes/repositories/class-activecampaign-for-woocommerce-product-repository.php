...
class Activecampaign_For_Woocommerce_Product_Repository {
	...
	/**
	 * Whether or not product catalog feature is enabled.
	 *
	 * @param ?array $ac_settings AC Settings array, otherwise we'll set it from db.
	 * @return boolean
	 */
	public static function is_enabled( $ac_settings = null ): bool {
		if ( ! $ac_settings ) {
			$ac_settings = get_option( ACTIVECAMPAIGN_FOR_WOOCOMMERCE_DB_OPTION_NAME );
		}

		if (
			isset( $ac_settings[ ACTIVECAMPAIGN_FOR_WOOCOMMERCE_PRODUCT_SYNC_ENABLED_NAME ] ) &&
			'0' === $ac_settings[ ACTIVECAMPAIGN_FOR_WOOCOMMERCE_PRODUCT_SYNC_ENABLED_NAME ]
			) {
			return false;
		}
