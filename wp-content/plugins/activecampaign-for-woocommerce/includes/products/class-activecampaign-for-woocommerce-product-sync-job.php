...
class Activecampaign_For_Woocommerce_Product_Sync_Job implements Executable {
	...
	const MAX_BULK_PRODUCT_PAGE_SIZE = 50;
	 ...
	 * @param mixed ...$args args.
	 */
	public function save_or_update_product( ...$args ) {
		$logger = new Logger();
		...
		try {
			...
			$this->logger->error(
				'There was an issue setting up product data for sync.',
				[
	...
	private function get_products_by_offset( $limit, $offset, $return_id_only ) {
		...
		try {
			$data = [
				'limit'   => (int) $limit,
				'offset'  => (int) $offset,
				'type'    => array( 'simple', 'variable', 'external', 'subscription', 'variable-subscription' ),
				'orderby' => 'none',
				'order'   => 'ASC',
			];
	...
	public function schedule_background_product_sync( $offset, $batch_limit ) {
		...
		try {
			...
			$logger->error(
				'There was an issue scheduling continued bulk product sync',
				[
					'message'  => $t->getMessage(),
					'trace'    => $t->getTrace(),
	 ...
	 * @return int
	 */
	public function product_cofe_sync( Sync_Status $status ) {
		/**
	...
	private function run_product_sync_process( Sync_Status $status ) {
		...
		try {
			...
			$this->logger->error(
				'Product sync failed for Offset:',
				[
					'message' => $t->getMessage(),
					'offset'  => $this->start_record,
					'limit'   => $this->batch_limit,
	...
	public function add_product_data( Sync_Status $status, WC_Product $product, $connection_id, array &$product_data, $parent = null ) {
		...
		// Bulk API should only handle pages of MAX_BULK_PRODUCT_PAGE_SIZE, so do API call and reset array when we get to that size.
		if ( count( $product_data ) === self::MAX_BULK_PRODUCT_PAGE_SIZE ) {
			try {
	...
	private function add_failed_product_to_status( Sync_Status $status, ?WC_Product $product = null ) {
		if ( $product && AC_Utilities::validate_object( $product, 'get_id' ) && ! empty( $product->get_id() ) ) {
	...
	private function cache_flush_extend_time() {
		$this->logger->debug( 'cache_flush_extend_time' );
		try {
			...
			$this->logger->debug(
				'Cache flush and time extend ran into a problem',
				[
					'message' => $t->getMessage(),
					'trace'   => $t->getTrace(),
				]
