...
use EasyAffiliate\Lib\Exception\Create as CreateException;
...
abstract class BaseModel {
  ...
  public function __get($name) {
    ...
    return apply_filters('esaf_get_model_attribute_'.$name, $this->cast_attr($name,$value), $this);
  }

  public function __set($name, $value) {
    $value = apply_filters('esaf_set_model_attribute_'.$name, $value, $this);
  ...
  public function load_from_sanitized_array($values) {
    $attr_keys = $this->get_attr_keys();
    $values = Utils::filter_array_keys($values, $attr_keys);
  ...
  public function load_from_get($unset_bools = false) {
    $this->load_from_request('get', $unset_bools);
  }

  public function load_from_post($unset_bools = false) {
    $this->load_from_request('post', $unset_bools);
  }
  ...
  protected function validate_not_null($var, $field = '') {
    if(is_null($var)) {
      throw new CreateException(sprintf(__('%s must not be empty', 'easy-affiliate'), $field));
    }
  }
  ...
  protected function validate_is_bool($var, $field = '') {
    if(!is_bool($var) && $var!=0 && $var!=1) {
      throw new CreateException(sprintf(__('%s must be true or false', 'easy-affiliate'), $field));
    }
  }

  protected function validate_is_array($var, $field = '') {
    if(!is_array($var)) {
      throw new CreateException(sprintf(__('%s must be an array', 'easy-affiliate'), $field));
    }
  }

  protected function validate_is_in_array($var, $lookup, $field = '') {
    if(is_array($lookup) && !in_array($var, $lookup)) {
      throw new CreateException(sprintf(__('%1$s must be %2$s NOT %3$s', 'easy-affiliate'), $field, implode( ' '.__('or', 'easy-affiliate').' ', $lookup), $var));
    }
  }

  protected function validate_is_url($var, $field = '') {
    if(!Utils::is_url($var)) {
      throw new CreateException(sprintf(__('%1$s (%2$s) must be a valid url', 'easy-affiliate'), $field, $var));
    }
  }

  protected function validate_is_currency($var, $min = 0.00, $max = null, $field = '') {
    if(!is_numeric($var) || $var < $min || (!is_null($max) && $var > $max)) {
      throw new CreateException(sprintf(__('%1$s (%2$s) must be a valid representation of currency', 'easy-affiliate'), $field, $var));
    }
  }

  protected function validate_is_numeric($var, $min = 0, $max = null, $field = '') {
    if(!is_numeric($var) || $var < $min || (!is_null($max) && $var > $max)) {
      throw new CreateException(sprintf(__('%1$s (%2$s) must be a valid number', 'easy-affiliate'), $field, $var));
    }
  }

  protected function validate_is_email($var, $field = '') {
    if(!Utils::is_email($var)) {
      throw new CreateException(sprintf(__('%1$s (%2$s) must be a valid email', 'easy-affiliate'), $field, $var));
    }
  }

  protected function validate_is_phone($var, $field = '') {
    if(!Utils::is_phone($var)) {
      throw new CreateException(sprintf(__('%1$s (%2$s) must be a valid phone number', 'easy-affiliate'), $field, $var));
    }
  }

  protected function validate_is_ip_addr($var, $field = '') {
    if(!Utils::is_ip($var)) {
      throw new CreateException(sprintf(__('%1$s (%2$s) must be a valid IP Address', 'easy-affiliate'), $field, $var));
    }
  }

  protected function validate_is_date($var, $field = '') {
    if(!Utils::is_date($var)) {
      throw new CreateException(sprintf(__('%1$s (%2$s) must be a valid date', 'easy-affiliate'), $field, $var));
    }
  }
  ...
  protected function validate_is_timestamp($var, $field = '') {
    if(empty($var) || !is_numeric($var)) {
      throw new CreateException(sprintf(__('%1$s (%2$s) must be a valid timestamp', 'easy-affiliate'), $field, $var));
    }
  }

  protected function validate_regex($pattern, $var, $field = '') {
    if(!preg_match($pattern, $var)) {
      throw new CreateException(sprintf(__('%1$s (%2$s) must match the regex pattern: %3$s', 'easy-affiliate'), $field, $var, $pattern));
    }
  }

  // abstract static methods throw a warning until PHP 7 so for now we'll just have
  // to keep track of this manually to ensure that each model implements it ... ugh
  //abstract public static function get_one($args);

  // Only necessary to pass classes around because we're still supporting < PHP 5.3 installs
  protected static function get_one_by_class($class, $args) {
    ...
    // Get the sub class ... doesn't work until PHP 5.3
  ...
  // abstract static methods throw a warning until PHP 7 so for now we'll just have
  // to keep track of this manually to ensure that each model implements it ... ugh
  //abstract public static function get_all($order_by='',$limit='',$args=array());
  ...
  // abstract static methods throw a warning until PHP 7 so for now we'll just have
  // to keep track of this manually to ensure that each model implements it ... ugh
  //abstract public static function get_count($args=array());
  ...
  public function get_attr_key($attr) {
    ...
    if(array_key_exists($attr, $attrs)) {
      $class = get_class($this);
      $model_str = strtolower(basename(str_replace('\\', '/', $class)));
      return "_wafp_{$model_str}_{$attr}";
    }
  ...
  public function get_attr_from_key($key) {
    ...
    if( preg_match("/^_wafp_{$model_str}_(.*)$/", $key, $m) &&
