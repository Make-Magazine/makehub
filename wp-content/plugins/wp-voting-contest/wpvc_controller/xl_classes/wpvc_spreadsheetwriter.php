...
class Wpvc_Spreadsheet_Excel_Writer
{
	//------------------------------------------------------------------
	...
	protected function initializeSheet($sheet_name)
	{
		...
		$this->sheets[$sheet_name] = (object)array(
			...
			'row_count' => 0,
			'file_writer' => new Wpvc_Spreadsheet_BuffererWriter($sheet_filename),
			'cell_formats' => array(),
		...
		$max_cell=Wpvc_Spreadsheet_Excel_Writer::xlsCell(self::EXCEL_2007_MAX_ROW, self::EXCEL_2007_MAX_COL);//XFE1048577
	...
	protected function writeCell(Wpvc_Spreadsheet_BuffererWriter &$file, $row_number, $column_number, $value, $cell_format)
	{
	...
	protected function writeStylesXML()
	{
		...
		$file = new Wpvc_Spreadsheet_BuffererWriter($temporary_filename);
	...
	{
		$temporary_filename = $this->tempFilename();
		$file = new Wpvc_Spreadsheet_BuffererWriter($temporary_filename, $fd_flags='w', $check_utf8=true);
		$file->write('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'."\n");
...
class Wpvc_Spreadsheet_BuffererWriter
{
	protected $fd=null;
	protected $buffer='';
	...
	public function __construct($filename, $fd_fopen_flags='w', $check_utf8=false)
	{
		...
		if ($this->fd===false) {
			Wpvc_Spreadsheet_Excel_Writer::log("Unable to open $filename for writing.");
		}
	}
	...
	protected function purge()
	{
		if ($this->fd) {
			if ($this->check_utf8 && !self::isValidUTF8($this->buffer)) {
				Wpvc_Spreadsheet_Excel_Writer::log("Error, invalid UTF8 encoding detected.");
