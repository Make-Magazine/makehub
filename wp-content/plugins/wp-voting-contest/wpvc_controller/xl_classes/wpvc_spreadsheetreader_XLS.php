<?php
	/****** Ohio web tech **********/
 	class Wpvc_SpreadsheetReader_XLS implements Iterator, Countable
	{
		...
		public function __construct($Filepath, array $Options = null)
		{
			...
			if (!class_exists('Wpvc_Spreadsheet_Excel_Reader'))
			{
				throw new Exception('SpreadsheetReader_XLS: Spreadsheet_Excel_Reader class not available');
			}

			$this -> Handle = new Wpvc_Spreadsheet_Excel_Reader($Filepath, false, 'UTF-8');

			if (function_exists('mb_convert_encoding'))
			{
				$this -> Handle -> wpvc_setUTFEncoder('mb');
			}
