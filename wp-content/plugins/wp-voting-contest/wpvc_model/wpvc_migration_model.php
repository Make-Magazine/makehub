...
if(!class_exists('Wpvc_Migration_Model')){
	class Wpvc_Migration_Model {
        public function __construct(){
            //add_action('init', array($this,'wpvc_get_poston_search'));
            //add_action('init', array($this,'wpvc_get_votes_count'));
        }

        public static function wpvc_get_votes_count(){
            global $wpdb;
            $total_posts = get_posts(array('post_type' => WPVC_VOTES_TYPE, 'numberposts' => -1, 'post_status' => 'any'));
            if(is_array($total_posts)){
                foreach($total_posts as $key=>$posts){
                    $post_id = $posts->ID;
                    $result = $wpdb->get_results("SELECT * FROM `".WPVC_VOTES_TBL."` WHERE post_id='".$post_id."'");
        ...
        public static function wpvc_check_migration_process(){
            global $wpdb;
			$result = $wpdb->get_results("SHOW COLUMNS FROM `".WPVC_VOTES_USER_CUSTOM_TABLE."` LIKE 'react_val'");
            $exists = $wpdb->num_rows;           
            return $exists;
        }
		...
		public static function wpvc_migrate_plugin_to_react(){
            ...
            $return_val = array('already_migrated'=>$exists,'completed_migration' => $completed_migration,'settings' => $setting_saved,'category'=>$category);
            if(is_array($backup)){
        ...
        public static function wpvc_backup_db() {
            ...
            if(is_array($upload_dir)){
                ...
                if(file_exists($backup_file_name)){
                    $return_val = array('backup_success' =>'1' ,'backup_path' =>$backup_file_name);
                }else{
                    $return_val = array('backup_success' =>'0' ,'backup_path' =>$backup_file_name);
                }
            }
            return $return_val;
        }
        ...
        public static function wpvc_migrate_settings(){
            ...
            $buy_vote_setting = get_option(OLD_BUYVOTES_SETTINGS);
            $video_settings = maybe_unserialize(get_option(OLD_VIDEO_EXTENSION_SETTINGS));
            ...
            if(!isset($setting_check['common']) && $new_setting_check==""){
                ...
                //Style
                $style = array('row_width'=>'3','direction'=>'row','column_width'=>'12','wpvc_spacing'=>'1');
                ...
                //color
                $color = array('votes_counter_font_size'=>'14',
                'votes_navigation_font_size'=>'14','votes_navigation_text_color'=>'#FFFFFF','votes_navigation_text_color_hover'=>'#ffffff',
                'votes_navigationbgcolor'=>'#305891','votes_list_active'=>'#F26E2A','votes_list_inactive'=>'#ffffff',
                'votes_grid_active'=>'#F26E2A','votes_grid_inactive'=>'#ffffff','vote_navbar_active_button_background'=>'#d83c30',
                'vote_navbar_mobile_font'=>'#ffffff',
                'votes_cont_title_font_size'=>'16','votes_cont_title_bgcolor'=>'#30598f','votes_cont_title_color'=>'#FFFFFF',
                'votes_cont_title_color_hover'=>'#F26E2A','votes_cont_title_color_grid'=>'#f26e2a','votes_cont_title_color_hover_grid'=>'#30598f',
                'votes_cont_desc_font_size'=>'16','votes_cont_dese_color'=>'#000000','votes_readmore_font_size'=>'14',
                'votes_readmore_fontcolor'=>'#F26E2A','votes_readmore_fontcolor_hover'=>'#000000',
                'votes_count_font_size'=>'16','votes_count_font_color'=>'#FFFFFF','votes_count_bgcolor'=>'#3276b1',
                'votes_button_font_size'=>'16','votes_button_font_color'=>'#FFFFFF','votes_button_font_color_hover'=>'#FFFFFF',
                'votes_button_bgcolor'=>'#305891','votes_button_bgcolor_hover'=>'#f26e2a','votes_highlight_button_bgcolor'=>'#008000',
                'votes_already_button_bgcolor'=>'#008000','votes_social_font_size'=>'16','votes_social_icon_color'=>'#F26E2A',
                ...
                $paymentsettings = array('paypal_settings' => $old_paypal_settings,'stripe_settings' => $old_stripe_settings,'paystack_settings' => $paystack_settings,'coinbase_settings' => $coinbase_settings );
        ...
        public static function create_custom_fields_for_react($custom_field,$img_contest,$musicfile=NULL){
            ...
            if(is_array($custom_field)){
                foreach($custom_field as $field){
                    if($field != '-1'){
                        $sql = "SELECT * FROM " .WPVC_VOTES_ENTRY_CUSTOM_TABLE." WHERE system_name ='".$field."'";
           ...
           if(is_array($custom_field)){
            ...
            foreach($custom_field as $custom){  
                ...
                if($type == 'FILE'){
                    ...
                    if(array_key_exists($system_name,$post_metas)){
                        if($system_name == 'contestant-ow_music_url'){
                            update_post_meta($post_id, '_ow_music_upload_attachment', $post_metas[$system_name][0]);
                            update_post_meta($post_id, 'contestant-ow_video_url', $post_metas[$system_name][0]);
                            update_post_meta($post_id, '_ow_music_upload_url', $post_metas[$system_name][0]); 
                            update_post_meta($post_id, 'contestant-ow_music_url_link', $post_metas[$system_name][0]);
                        }elseif($system_name == 'contestant-ow_music_url_link'){
                            update_post_meta($post_id, '_ow_music_upload_attachment', $post_metas[$system_name][0]);
                            update_post_meta($post_id, 'contestant-ow_video_url', $post_metas[$system_name][0]);
                            update_post_meta($post_id, '_ow_music_upload_url', $post_metas[$system_name][0]); 
                            update_post_meta($post_id, 'contestant-ow_music_url_link', $post_metas[$system_name][0]);
                        }elseif($system_name == 'contestant-ow_video_url'){
                            update_post_meta($post_id, 'contestant-ow_video_url',$post_metas[$system_name][0]);
                            update_post_meta($post_id, '_ow_video_upload_url', $post_metas[$system_name][0]);
                        } 
                    }   
                }                  
            }
        ...
        public static function wpvc_voting_get_all_custom_fields($term_id=NULL,$video_opt=NULL){
			...
			if($term_id!=''){
				...
				$ctg_options = get_option($term_id . '_' . WPVC_OLD_VOTES_SETTINGS);
				$image_contest = $ctg_options['imgcontest'];
				$musicfileenable = $ctg_options['musicfileenable'];
				$ctg_opt = $ctg_options['custom_field_names'];
				
				if(is_array($ctg_opt)){
				  foreach($ctg_opt as $ctg){
					if($ctg!='-1')
					  $comma .= "'".$ctg."',";
				  }
				  if($image_contest == 'video'){
					if($video_opt=='on'){
					  $comma .= "'contestant-ow_video_url','contestant-ow_video_upload_url',";
					}
					elseif($ow_video_extension==null)
					  $comma .= "'contestant-ow_video_url',";
					else{
					  $comma .= "'contestant-ow_video_upload_url',";
					}
					
				  }else if($image_contest == 'music'){
					if($musicfileenable == "on")
					  $comma .= "'contestant-ow_music_url',";
					else
					  $comma .= "'contestant-ow_music_url_link',";
				  }
				  $comma .= "'contestant-email_address'";
				 $sql = "SELECT * FROM " .WPVC_VOTES_ENTRY_CUSTOM_TABLE." WHERE system_name IN (".$comma.") AND delete_time = 0 order by sequence";
...
die("<h2>".__('Failed to load Voting Migration model')."</h2>");
