...
class Utils {
  ...
  */
  ...
  public static function error_log($error) {
    ...
    error_log(sprintf(__("*** MemberPress Gifting Error\n==========\n%s\n==========\n", 'memberpress-gifting'),$error));
  }
  ...
  public static function debug_log($message) {
    ...
    if(defined('WP_MPCS_DEBUG') && WP_MPCS_DEBUG) {
      error_log(sprintf(__('*** MemberPress Courses Debug: %s', 'memberpress-gifting'),$message));
    }
  }
  ...
  public static function send_notices($obj, $user_class=null, $admin_class=null, $force=false) {
    ...
    try {
      if( !is_null($user_class) ) {
        $uemail = EmailFactory::fetch($user_class);
        $uemail->to = $usr->formatted_email();

        if($user_class == base\EMAILS_NAMESPACE.'\\'.'GiftClaimEmail'){
          $current_user = self::get_currentuserinfo();
          $to_email = isset($_POST['giftee_email']) && is_email($_POST['giftee_email']) ? $_POST['giftee_email'] : '';
          $from_name = isset($_POST['gifter_name']) ? sanitize_text_field($_POST['gifter_name']) : '';
          $headers = array('From: '.$from_name.' <'. $current_user->user_email .'>');

          if(isset($_POST['action']) && $_POST['action'] == 'mpgft_send_gift_email'){
            $uemail->to = $to_email;
            $uemail->headers = $headers;
          }
        }
      ...
      if( !is_null($admin_class) ) {
        $aemail = EmailFactory::fetch($admin_class);
