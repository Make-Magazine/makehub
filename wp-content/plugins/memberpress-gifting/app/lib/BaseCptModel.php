...
abstract class BaseCptModel extends BaseBuiltinModel {
  ...
  public function destroy() {
    ...
    if(false===$res) {
      throw new CreateException(sprintf(__( 'This was unable to be deleted.', 'memberpress-gifting')));
    }
  ...
  public static function all($class, $reset_transients = false) {
    ...
    $r          = new \ReflectionClass(base\MODELS_NAMESPACE.'\\'.$class);
    $cpt        = $r->getStaticPropertyValue('cpt');
    $models     = array();
    $transient  = 'mpgft_all_models_for_class_'.strtolower($class);

    $use_transient_cache = apply_filters('mpgft-cpt-all-use-transient-cache', true, $cpt, $class);
