...
class LicenseManager {
	...
	public function activate_license( $license_key ) {
		...
		try {
			...
			if ( ! $response['_raw']['success'] ) {
				throw new Exception( esc_html__( 'Could not get information on products associated with this license.', 'gk-gravityrevisions' ) );
			}
		} catch ( Exception $e ) {
			throw new Exception( $e->getMessage() );
		}
	...
	public function modify_license_data_for_frontend_output( $license ) {
		...
		if ( ! ctype_alpha( $expiry ) ) {
			...
			$expiry = $expired
				? human_time_diff( $expiry, current_time( 'timestamp' ) ) . ' ' . esc_html_x( 'ago', 'Indicates "time ago"', 'gk-gravityrevisions' )
