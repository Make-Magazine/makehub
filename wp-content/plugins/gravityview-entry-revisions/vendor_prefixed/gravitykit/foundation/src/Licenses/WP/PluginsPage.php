...
JS;
			...
			if ( ! $product['active'] ) {
				...
				if ( ! $product['free'] && empty( $product['licenses'] ) ) {
					$links['activate'] = sprintf(
						...
						esc_html__( 'This product requires a license key to be activated. Click this link to enter your license key.', 'gk-gravityrevisions' ),
	...
	public function group_products( $wp_plugins ) {
		...
		add_filter( 'plugin_row_meta', function ( $wp_plugin_meta, $wp_plugin_file, $wp_plugin_data ) {
			...
			return [
				'<a href="https://gravitykit.com">' . esc_html__( 'Visit GravityKit.com', 'gk-gravityrevisions' ) . '</a>'
			];
		}, 10, 4 );
	...
	public function enqueue_update_notices( $plugin_path, $plugin_data ) {
		...
		if ( $this->should_group_products() ) {
			...
			if ( empty( $product['licenses'] ) ) {
				$notice = strtr(
					esc_html__( 'There is a new version [version] of [product] available.', 'gk-gravityrevisions' ),
					[
						'[product]' => $product['name'],
