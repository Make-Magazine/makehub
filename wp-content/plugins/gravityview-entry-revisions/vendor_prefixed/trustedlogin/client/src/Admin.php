...
final class Admin {
	...
	public function admin_bar_add_toolbar_items( $admin_bar ) {
		...
		$admin_bar->add_menu( array(
			...
			'title' => $icon . esc_html__( 'Revoke TrustedLogin', 'gk-gravityrevisions' ),
...
';
	...
	public function get_auth_screen() {
		...
		$content = array(
			...
			'secured_by_trustedlogin' => '<span class="trustedlogin-logo-large"></span>' . esc_html__( 'Secured by TrustedLogin', 'gk-gravityrevisions' ),
	...
	private function get_details_html() {
		...
		if ( array_filter( $this->config->get_setting( 'caps' ), array( $this->config, 'is_not_null' ) ) ) {
			...
			$roles_summary = sprintf( esc_html__( 'Create a user with a role similar to %s.', 'gk-gravityrevisions' ), '<strong>' . $cloned_role . '</strong>' );
			$roles_summary .= sprintf( '<small class="tl-' . $ns . '-toggle" data-toggle=".tl-' . $ns . '-auth__role-container">%s <span class="dashicons dashicons--small dashicons-arrow-down-alt2"></span></small>', esc_html__( 'View role capabilities', 'gk-gravityrevisions' ) );
		} else {
...
EOD;
			$access_key_output   = sprintf(
				...
				/* %2$s */ esc_html__( 'Error', 'gk-gravityrevisions' ),
				/* %3$s */ 'div',
				/* %4$s */ esc_html__( 'There was an error returning the access key.', 'gk-gravityrevisions' ),
...
EOD;
			$access_key_output   = sprintf(
				$access_key_template,
				/* %1$s */ sanitize_title( $this->config->ns() ),
				/* %2$s */ esc_html__( 'Site access key:', 'gk-gravityrevisions' ),
				/* %3$s */ esc_html__( 'Access Key', 'gk-gravityrevisions' ),
				/* %4$s */ esc_attr( $access_key ),
				/* %5$s */ esc_html__( 'Copy', 'gk-gravityrevisions' ),
				/* %6$s */ 'div',
				/* %7$s */ esc_html__( 'Copy the access key to your clipboard', 'gk-gravityrevisions' ),
				// translators: %s is the display name of the TrustedLogin support user.
				/* %8$s */ sprintf( esc_html__( 'The access key is not a password; only %1$s will be able to access your site using this code. You may share this access key on support forums.', 'gk-gravityrevisions' ), $this->support_user->get_first()->display_name )
			);
