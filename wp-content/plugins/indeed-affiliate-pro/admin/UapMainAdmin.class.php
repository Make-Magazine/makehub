		...
		*/
		...
		private function print_head($tab){
			...
			$data['menu_items'] = array(
											...
											'notifications' => __('Email Notifications', 'uap'),
					...
					if ($batch_id){
						...
						return TRUE;
							...
							if ( $errorDetails ){
									...
									$errorDetails = __( 'The Payment cannot be proceed because of the payment settings.', 'uap' );
							}
							$data = array(
									'payment_type' 				=> 'paypal',
									'transaction_id' 			=> '',
		...
		private function print_settings(){
			...
			$data['submenu'] = array(
					...
					admin_url('admin.php?page=ultimate_affiliates_pro&tab=settings&subtab=notification_settings') => __('Notifications Settings', 'uap'),
					...
					admin_url('admin.php?page=ultimate_affiliates_pro&tab=settings&subtab=access') => __('WP Dashboard Access', 'uap'),
					admin_url('admin.php?page=ultimate_affiliates_pro&tab=settings&subtab=captcha') => __('reCaptcha Setup', 'uap'),
			 ...
			 */
			 ...
			 if (isset($_GET['add_edit'])){
				...
				if (!empty($_POST['save'])){
						...
						if ($saved<1){
							$data['errors'] = __('Be sure that you have filled all the reguired fields: Code, Affiliate.', 'uap');
						}
		...
		private function print_mlm_view_affiliate_children(){
	  	...
	  	$affiliate_full_name = $indeed_db->get_full_name_of_user($affiliate_id);
			if($affiliate_full_name == '') $affiliate_full_name = 	$affiliate_name;
			$affiliate_avatar = uap_get_avatar_for_uid($affiliate_id);
			...
			if (empty($data['parent'])){
					...
					if($data['parent_full_name'] == '') $data['parent_full_name'] = $data['parent'];
		...
		public function wooCouponsSettings()
		{
				...
				$data = [
									...
									'types'			 => [
																		'flat' 					=> __( 'Flat ', 'uap') . '(' . get_option('uap_currency') .')',
																		'percentage'		=> __( 'Percentage ', 'uap') . '(%)',
									],
