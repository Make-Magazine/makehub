...
class Batch {
	...
	public static function create( $args ) {
		...
		if ( ! $id ) {
			return new \WP_Error( 'gravityview/import/errors/fatal', __( 'Could not create import batch in database.', 'gravityview-importer' ) );
		}
	...
	public static function update( $batch ) {
		...
		if ( ! Batch::get( $batch['id'] ) ) {
			return new \WP_Error( 'gravityview/import/errors/fatal', __( 'Could not save batch in database.', 'gravityview-importer' ) );
		}
	...
	public static function validate( $args ) {
		...
		if ( $args['id'] && is_null( Batch::get( $args['id'] ) ) ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_id', __( 'Invalid batch ID supplied.', 'gravityview-importer' ) );
		}

		if ( ! in_array( $args['status'], $user = $schema['properties']['status']['extra']['user'] ) ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_status', sprintf( __( 'Invalid status argument. Allowed: %s', 'gravityview-importer' ), implode( ' ', $user ) ) );
		}

		if ( $args['form_id'] ) {
			if ( ! \GFAPI::form_id_exists( $args['form_id'] ) ) {
				return new \WP_Error( 'gravityview/import/errors/invalid_form', __( 'Invalid form ID supplied.', 'gravityview-importer' ) );
			}
			...
			foreach ( $args['schema'] as $id => $rule ) {
				...
				if ( $form ) {
					if ( in_array( $rule['field'], $seen_fields ) ) {
						...
						return new \WP_Error( 'gravityview/import/errors/invalid_rule', sprintf( __( 'Rule %d duplicate field mismatch.', 'gravityview-importer' ), $id ), array( 'rule' => $rule ) );
					}
		...
		if ( $form && $args['feeds'] ) {
			...
			foreach ( $args['feeds'] as $feed_id ) {
				if ( ! in_array( $feed_id, $feed_ids ) ) {
					return new \WP_Error( 'gravityview/import/errors/invalid_feed', sprintf( __( 'Invalid feed ID %s provided for form ID %s.', 'gravityview-importer' ), $feed_id, $form['id'] ), array( 'feed_id' => $feed_id, 'form_id' => $form['id'] ) );
				}
			}
		} else if ( $args['feeds'] ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_feed', __( '"feeds" parameter must be empty for new forms.', 'gravityview-importer' ) );
		}
		...
		if ( empty( $args['source'] ) ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_source', __( '"source" parameter is required', 'gravityview-importer' ) );
		}

		if ( $args['flags'] ) {
			if ( $not_allowed = array_diff( $args['flags'], $schema['properties']['flags']['anyOf'] ) ) {
				return new \WP_Error( 'gravityview/import/errors/invalid_flags', sprintf( __( 'Unknown flags: %s', 'gravityview-importer' ), implode( ' ', $not_allowed ) ) );
			}
		}

		if ( array_keys( $args['meta'] ) != array( 'rows', 'columns', 'excerpt' ) ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_meta', __( 'Invalid meta structure', 'gravityview-importer' ), array( 'meta' => $args['meta'] ) );
		}
	...
	public static function validate_rule( $id, $rule, $form ) {
		...
		if ( $not_allowed = array_diff( array_keys( $rule ), array_keys( $defaults ) ) ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_args', sprintf( __( 'Rule %d arguments not allowed here: %d', 'gravityview-importer' ), $id, implode( ' ', $not_allowed ) ) );
		}
		...
		if ( ! is_numeric( $rule['column'] ) || $rule['column'] < 0 ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_rule', sprintf( __( 'Rule %d invalid column number.', 'gravityview-importer' ), $id ) );
		}

		if ( ! $rule['field'] ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_rule', sprintf( __( 'Rule %d field cannot be empty.', 'gravityview-importer' ), $id ) );
		}

		if ( ! $is_meta && $form ) {
			if ( is_numeric( $rule['field'] ) ) {
				if ( ! $field = \GFFormsModel::get_field( $form, $rule['field'] ) ) {
					return new \WP_Error( 'gravityview/import/errors/invalid_rule', sprintf( __( 'Rule %d invalid field ID for form.', 'gravityview-importer' ), $id ) );
				}
			...
			if ( ! Core::is_entry_column( $rule['field'] ) && ! \GF_Fields::exists( $type ) ) {
				return new \WP_Error( 'gravityview/import/errors/invalid_rule', sprintf( __( 'Rule %d unknown field type.', 'gravityview-importer' ), $id ) );
			}
		}
		...
		foreach ( (array)$rule['flags'] as $flag ) {
			if ( ! in_array( $flag, $allowed = $schema['definitions']['rule']['properties']['flags']['anyOf'] ) ) {
				return new \WP_Error( 'gravityview/import/errors/invalid_rule', sprintf( __( 'Rule %d unknown flag. Allowed: %s', 'gravityview-importer' ), $id, implode( ' ', $allowed ) ) );
			}
		}

		foreach ( (array)$rule['meta'] as $meta => $_ ) {
			if ( ! in_array( $meta, $allowed = array_keys( $schema['definitions']['rule']['properties']['meta']['properties'] ) ) ) {
				return new \WP_Error( 'gravityview/import/errors/invalid_rule', sprintf( __( 'Rule %d unknown meta. Allowed: %s', 'gravityview-importer' ), $id, implode( ' ', $allowed ) ) );
			}
		}
	...
	public static function validate_condition( $condition ) {
		...
		if ( $not_allowed = array_diff( array_keys( $condition ), array_keys( $defaults ) ) ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_args', sprintf( __( 'Condition arguments not allowed here: %s', 'gravityview-importer' ),  implode( ' ', $not_allowed ) ) );
		}
		...
		if ( is_numeric( $condition['column'] ) ) {
			if ( $condition['column'] < 0 ) {
				return new \WP_Error( 'gravityview/import/errors/invalid_condition', __( 'Condition invalid column number.', 'gravityview-importer' ) );
			}

			if ( ! in_array( $condition['op'], $comparators ) ) {
				return new \WP_Error( 'gravityview/import/errors/invalid_condition', __( 'Condition invalid operation.', 'gravityview-importer' ) );
			}

			if ( ! is_string( $condition['value'] ) && ! is_numeric( $condition['value'] ) ) {
				return new \WP_Error( 'gravityview/import/errors/invalid_condition', __( 'Condition invalid literal value.', 'gravityview-importer' ) );
			}
		...
		if ( empty( $condition['column'] ) && is_array( $condition['value'] ) ) {
			if ( ! in_array( $condition['op'], $boolean_logic ) ) {
				return new \WP_Error( 'gravityview/import/errors/invalid_condition', __( 'Invalid boolean operator for nested conditions.', 'gravityview-importer' ) );
			}
		...
		return new \WP_Error( 'gravityview/import/errors/invalid_condition', __( 'Malformed condition. Missing column or condition chain.', 'gravityview-importer' ) );
	}
	...
	public static function validate_metacolumn( $id, $column, $form ) {
		...
		if ( $not_allowed = array_diff( array_keys( $column ), array_keys( $defaults ) ) ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_args', sprintf( __( 'Column meta %d arguments not allowed here: %d', 'gravityview-importer' ), $id, implode( ' ', $not_allowed ) ) );
		}

		$rule = shortcode_atts( $defaults, $column );

		if ( ! is_string( $column['title'] ) ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_columnmeta', sprintf( __( 'Column meta %d invalid column title.', 'gravityview-importer' ), $id ) );
		}

		if ( $form ) {
			if ( is_numeric( $rule['field'] ) ) {
				if ( ! $field = \GFFormsModel::get_field( $form, $rule['field'] ) ) {
					return new \WP_Error( 'gravityview/import/errors/invalid_columnmeta', sprintf( __( 'Column meta %d invalid field ID for form.', 'gravityview-importer' ), $id ) );
				}
			...
			if ( ! \GF_Fields::exists( $rule['field'] ) ) {
				return new \WP_Error( 'gravityview/import/errors/invalid_columnmeta', sprintf( __( 'Column meta %d unknown field type.', 'gravityview-importer' ), $id ) );
			}
		}
	...
	public static function delete( $batch_id ) {
		...
		if ( ! $batch = Batch::get( $batch_id ) ) {
			return new \WP_Error( 'gravityview/import/errors/not_found', __( 'Unknown batch ID', 'gravityview-importer' ) );
		}

		if ( in_array( $batch['status'], array( 'parsing', 'processing', 'halt', 'rollback' ) ) ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_status', sprintf( __( 'Batch is not in stop state: %s', 'gravityview-importer' ), $batch['status'] ) );
		}
