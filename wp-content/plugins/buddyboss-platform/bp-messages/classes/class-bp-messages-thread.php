	...
	public static function get_threads_for_user( $args = array() ) {
		...
		if ( false === bp_disable_group_messages() || ! bp_is_active( 'groups' ) ) {
			...
			if(!empty($message_ids)){
				...
				error_log('empty. Query is '."SELECT DISTINCT message_id from {$bp->messages->table_name_meta} WHERE 1 = 1 {$group_ids_sql} AND message_id IN ( SELECT DISTINCT message_id from {$bp->messages->table_name_meta} WHERE meta_key  = 'group_message_users' and meta_value = 'all' AND message_id in ( SELECT DISTINCT message_id from {$bp->messages->table_name_meta} where meta_key  = 'group_message_type' and meta_value = 'open' ) ) ");
				$sub_query = "";
			}
