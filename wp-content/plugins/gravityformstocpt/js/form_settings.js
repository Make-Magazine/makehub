	...
	$( document ).ready( function() {
		...
		$( 'select[name="_gaddon_setting_postDate"]' ).on( 'change', function() {
			...
			switch ( $( this ).val() ) {
				...
				case 'custom':
					$( this ).siblings( '.gform_formstocpt_date_custom' ).show();
					break;

				case 'field':
					$( this ).siblings( '.gform_formstocpt_date_field' ).show();
					break;
		...
		$( '#postAuthor' ).select2( {
		    allowClear:  false,
		    placeholder: gform_formstocpt_form_settings_strings.select_user,
			ajax:        {
				url:      ajaxurl,
				dataType: 'json',
				delay:    250,
				data:     function( params ) {
					return {
						action: 'gform_formstocpt_author_search',
						nonce:  gform_formstocpt_form_settings_strings.nonce_author,
						query:  params.term,
					}
				}
			}
