 ...
 */
	...
	window.GFAPCUtils = {
		...
		'getPostDateOptionFieldByName': function ( $postDateSelect, option ) {
			...
			if ( GFVersion >= '2.5'  ) {
				return $postDateSelect.parent().siblings( '.gform_formsgftocpt_' + option );
			}

			return $postDateSelect.siblings( '.gform_formsgftocpt_' + option );
		},
		...
		'getPostDateOptionFields': function ( $postDateSelect ) {
			if ( GFVersion >= '2.5' ) {
				return $postDateSelect.parent().siblings( 'div[class^=gform_formsgftocpt_]' );
			}
			return $postDateSelect.siblings(  'div[class^=gform_formsgftocpt_]' );
		},
