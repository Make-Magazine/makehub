...
/**
 * Gravity Forms - Form to CPT Add-On.
 *
 * @since     1.0
 ...
 */
class GF_Forms_To_CPT extends GFFeedAddOn {
	...
	/**
	 * Defines the version of the Gravity Forms - Form to CPT Add-On.
	 *
	 * @since  1.0
	 * @access protected
	 * @var    string $_version Contains the version, defined from gravityformsformstocpt.php
	 */
	protected $_version = GF_FORMSTOCPT_VERSION;
	 ...
	 */
	protected $_slug = 'gravityformsformstocpt';
	 ...
	 */
	protected $_path = 'gravityformsformstocpt/gravityformsformstocpt.php';
	 ...
	 */
	protected $_title = 'Gravity Forms - Forms to CPT Make: Add-On';
	 ...
	 */
	protected $_short_title = 'Form to CPT';
	 ...
	 */
	protected $_capabilities_settings_page = 'gravityforms_gravityformsformstocpt';
	 ...
	 */
	protected $_capabilities_form_settings = 'gravityforms_gravityformsformstocpt';
	 ...
	 */
	protected $_capabilities_uninstall = 'gravityforms_gravityformsformstocpt_uninstall';

	/**
	 * Defines the capabilities needed for the Gravity Forms - Form to CPT Add-On
	 *
	 * @since  1.0
	 * @access protected
	 * @var    array $_capabilities The capabilities needed for the Add-On
	 */
	protected $_capabilities = array( 'gravityforms_gravityformsformstocpt', 'gravityforms_gravityformsformstocpt_uninstall' );
	...
	public function init() {
		...
		$this->add_delayed_payment_support(
			array(
				'option_label' => esc_html__( 'Create post only when payment is received.', 'gravityforms_gravityformsformstocpt' )
			)
		);
	...
	public function init_ajax() {
		...
		add_action( 'wp_ajax_gform_gravityformsformstocpt_taxonomy_search', array( $this, 'get_taxonomy_map_search_results' ) );
		add_action( 'wp_ajax_gform_gravityformsformstocpt_author_search', array( $this, 'get_post_author_search_results' ) );
	...
	public function styles() {
		...
		$styles = array(
			array(
				'handle'  => 'gform_gravityformsformstocpt_form_settings',
			...
			array(
				'handle'  => 'gform_gravityformsformstocpt_select2',
	...
	public function scripts() {
		...
		$scripts = array(
			...
			array(
				'handle'  => 'gform_gravityformsformstocpt_taxonomy_map',
				'deps'    => array( 'gform_gravityformsformstocpt_select2' ),
			...
			array(
				'handle'  => 'gform_gravityformsformstocpt_form_settings',
				'deps'    => array( 'gform_gravityformsformstocpt_taxonomy_map' ),
				...
				'strings' => array(
					'nonce_author' => wp_create_nonce( 'gform_gravityformsformstocpt_author_search' ),
					'nonce_search' => wp_create_nonce( 'gform_gravityformsformstocpt_taxonomy_search' ),
					'select_user'  => wp_strip_all_tags( __( 'Select a User', 'gravityformsformstocpt' ) ),
				),
			),
		);
	 ...
	 * @access public
	 *
	 * @uses   GF_Forms_To_CPT::feed_settings_fields_create_post()
		...
		$base_fields = array(
			array(
				...
				'fields' => array(
					array(
						'name'          => 'feedName',
						'label'         => esc_html__( 'Feed Name', 'gravityformsformstocpt' ),
						'type'          => 'text',
						'required'      => true,
						'class'         => 'medium',
						...
						'tooltip'       => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Name', 'gravityformsformstocpt' ),
							esc_html__( 'Enter a feed name to uniquely identify this setup.', 'gravityformsformstocpt' )
						),
					),
					array(
						'name'  => 'action',
						'label' => esc_html__( 'Action', 'gravityformsformstocpt' ),
						'type'  => 'hidden',
						'value' => 'post-create',
					),
					array(
			...
			array(
				'fields' => array(
					array(
						'name'           => 'feedCondition',
						'type'           => 'feed_condition',
						'label'          => esc_html__( 'Conditional Logic', 'gravityformsformstocpt' ),
						'checkbox_label' => esc_html__( 'Enable', 'gravityformsformstocpt' ),
						'instructions'   => esc_html__( 'Create if', 'gravityformsformstocpt' ),
						'tooltip'        => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Conditional Logic', 'gravityformsformstocpt' ),
							esc_html__( 'When conditional logic is enabled, form submissions will only be created when the condition is met. When disabled, all form submissions will be created.', 'gravityformsformstocpt' )
						),
					),
				),
			),
	...
	/**
	 * Get a list of graity form events for the current form.
	 *
	 * @since  Unknown
	 * @access public
	 *
	 ...
	 */
	public function get_gravityform_events( $form) {
	...
	/**
	 * Get the GF events for the current form.
	 *
	 * @since  Unknown
	 * @access public
	 *
	 * @param array $form The current Form Object.
	 *
	 * @return array Events available within the form.
	 */
	 ...
	 * @uses GF_Forms_To_CPT::get_post_formats_as_choices()
	 * @uses GF_Forms_To_CPT::get_post_meta_field_map()
	 * @uses GF_Forms_To_CPT::get_post_statuses_as_choices()
	 * @uses GF_Forms_To_CPT::get_post_types_as_choices()
	 * @uses GF_Forms_To_CPT::get_taxonomies_as_feed_settings_fields()
	 * @uses GF_Forms_To_CPT::get_thumbnail_fields_as_choices()
	 *
	 * @return array
	 */
	public function feed_settings_fields_create_post() {
		...
		$post_date_choices = array(
			array(
				'label' => esc_html__( 'Entry Date', 'gravityformsformstocpt' ),
				'value' => 'entry',
			),
			array(
				'label' => esc_html__( 'Date & Time Fields', 'gravityformsformstocpt' ),
				'value' => 'field',
			),
			array(
				'label' => esc_html__( 'Custom Date & Time', 'gravityformsformstocpt' ),
				'value' => 'custom',
			),
		);
		...
		$fields = array(
			'settings' => array(
				'id'     => 'postSettings',
				'title'  => esc_html__( 'Post Settings', 'gravityformsformstocpt' ),
				'fields' => array(
					array(
						'name'          => 'postVisibility',
						'label'         => esc_html__( 'Visibility', 'gravityformsformstocpt' ),
						'type'          => 'radio',
						'required'      => true,
						'horizontal'    => true,
						...
						'choices'       => array(
							array(
								'label' => esc_html__( 'Public', 'gravityformsformstocpt' ),
								'value' => 'public',
							),
							array(
								'label' => esc_html__( 'Password Protected', 'gravityformsformstocpt' ),
								'value' => 'password-protected',
							),
						),
						'tooltip'       => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Post Visibility', 'gravityformsformstocpt' ),
							esc_html__( 'Visibility determines who can read the post.', 'gravityformsformstocpt' )
						),
					),
					array(
						'name'       => 'postPassword',
						'label'      => esc_html__( 'Password', 'gravityformsformstocpt' ),
						'type'       => 'text',
						'class'      => 'medium merge-tag-support mt-position-right mt-hide_all_fields',
						'dependency' => array( 'field' => 'postVisibility', 'values' => array( 'password-protected' ) ),
						'tooltip'    => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Post Password', 'gravityformsformstocpt' ),
							esc_html__( 'Users will be required to enter the chosen post password to view the post content', 'gravityformsformstocpt' )
						),
					),
					array(
						'name'     => 'postType',
						'label'    => esc_html__( 'Type', 'gravityformsformstocpt' ),
						'type'     => 'select',
						'required' => true,
						...
						'tooltip'  => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Post Type', 'gravityformsformstocpt' ),
							esc_html__( 'Select one of the defined WordPress post types for the post.', 'gravityformsformstocpt' )
						),
					),
					array(
						'name'          => 'postStatus',
						'label'         => esc_html__( 'Status', 'gravityformsformstocpt' ),
						'type'          => 'select',
						'required'      => true,
						...
						'tooltip'       => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Post Status', 'gravityformsformstocpt' ),
							esc_html__( 'Select the status for the post. If published status is selected and the post date is in the future, it will automatically be changed to scheduled.', 'gravityformsformstocpt' )
						),
					),
					array(
						'name'                => 'postDate',
						'label'               => esc_html__( 'Date', 'gravityformsformstocpt' ),
						'type'                => 'post_date',
						'required'            => true,
						...
						'tooltip'       => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Post Date', 'gravityformsformstocpt' ),
							esc_html__( 'Define the post publish date. Custom date & time can be a specific date and time or a relative time (e.g. "today", "next week").', 'gravityformsformstocpt' )
						),
					),
					array(
						'name'          => 'postAuthor',
						'label'         => esc_html__( 'Author', 'gravityformsformstocpt' ),
						'type'          => 'select',
						'required'      => false,
						...
						'tooltip'       => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Post Author', 'gravityformsformstocpt' ),
							esc_html__( 'Select the user to be assigned to the post.', 'gravityformsformstocpt' )
						),

					),
					array(
						'name'    => 'postDiscussion',
						'label'   => esc_html__( 'Discussion', 'gravityformsformstocpt' ),
						'type'    => 'checkbox',
						'choices' => array(
							array(
								'label' => esc_html__( 'Allow Comments', 'gravityformsformstocpt' ),
								'name'  => 'postComments',
							),
							array(
								'label' => esc_html__( 'Allow Trackbacks and Pingbacks', 'gravityformsformstocpt' ),
								'name'  => 'postPingbacks',
							),
						),
					),
			...
			'content'  => array(
				'title'  => esc_html__( 'Post Content', 'gravityformsformstocpt' ),
				'fields' => array(
					array(
						'name'          => 'postTitle',
						'label'         => esc_html__( 'Title', 'gravityformsformstocpt' ),
						'type'          => 'text',
						'required'      => true,
						'class'         => 'medium merge-tag-support mt-position-right mt-hide_all_fields',
					...
					array(
						'name'          => 'postContent',
						'label'         => esc_html__( 'Content', 'gravityformsformstocpt' ),
						'type'          => 'textarea',
						'required'      => true,
						'class'         => 'medium merge-tag-support mt-position-right mt-hide_all_fields',
						...
						'tooltip'       => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Post Content', 'gravityformsformstocpt' ),
							esc_html__( "Define the post's content. File upload field merge tags used within the post content will automatically have their files uploaded to the media library and associated with the post.", 'gravityformsformstocpt' )
						),

					),
					array(
						'name'        => 'postMetaFields',
						'label'       => esc_html__( 'Custom Fields', 'gravityformsformstocpt' ),
						'type'        => 'generic_map',
						'merge_tags'  => true,
						'key_field'   => array(
							'choices'     => $this->get_post_meta_field_map(),
							'placeholder' => esc_html__( 'Custom Field Name', 'gravityformsformstocpt' ),
							'title'       => esc_html__( 'Name', 'gravityformsformstocpt' ),
						),
						'value_field' => array(
							'choices'      => 'form_fields',
							'custom_value' => true,
							'placeholder'  => esc_html__( 'Custom Field Value', 'gravityformsformstocpt' ),
						),
						'tooltip'       => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Custom Fields', 'gravityformsformstocpt' ),
							esc_html__( 'Map form values to post meta using an existing meta key or defining a new one.', 'gravityformsformstocpt' )
						),
					),
				),
			),
		...
		if ( $post_type_obj && post_type_supports( $post_type_obj->name, 'post-formats' ) && count( $post_formats ) > 1 ) {
			...
			$field = array(
				array(
					'name'    => 'postFormat',
					'label'   => esc_html__( 'Format', 'gravityformsformstocpt' ),
					'type'    => 'select',
		 ...
		 */
		if ( apply_filters( 'gform_gravityformsformstocpt_excerpt', false ) ) {

			// Prepare field.
			$field = array(
				array(
					'name'          => 'postExcerpt',
					'label'         => esc_html__( 'Excerpt', 'gravityformsformstocpt' ),
					'type'          => 'textarea',
					'class'         => 'medium merge-tag-support mt-position-right mt-hide_all_fields',
		...
		if ( $post_type_obj && $upload_fields = $this->get_file_fields_as_choices() ) {
			...
			if ( post_type_supports( $post_type_obj->name, 'thumbnail' ) && ! empty( $single_choices ) ) {
				...
				$single_choices = array_merge(
					array(
						array(
							'value' => '',
							'label' => esc_html__( 'Select a File Upload Field', 'gravityformsformstocpt' ),
						),
					),
					$single_choices
				);
				...
				$thumbnail_field = array(
					array(
						'name'    => 'postThumbnail',
						'label'   => esc_html__( 'Featured Image', 'gravityformsformstocpt' ),
						'type'    => 'select',
						'class'   => 'medium',
						'choices' => $single_choices,
						'tooltip' => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Featured Image', 'gravityformsformstocpt' ),
							esc_html__( "Select a file upload field to use for the post's featured image. Only file upload fields limited to a single file are permitted.", 'gravityformsformstocpt' )
						),
					),
				);
			...
			$media_field = array(
				array(
					'name'        => 'postMedia[]',
					'label'       => esc_html__( 'Media Library', 'gravityformsformstocpt' ),
					'type'        => 'select',
					'class'       => 'medium',
					'multiple'    => true,
					'placeholder' => esc_attr__( 'Select File Upload Fields', 'gravityformsformstocpt' ),
					'choices'     => $upload_fields,
					'tooltip'       => sprintf(
						'<h6>%s</h6>%s',
						esc_html__( 'Media Library', 'gravityformsformstocpt' ),
						esc_html__( 'Select file upload fields whose files should be uploaded to the media library and assigned to the post.', 'gravityformsformstocpt' )
					),
				),
			);
		...
		if ( $taxonomies ) {
			...
			$fields['taxonomies'] = array(
				'title'      => esc_html__( 'Taxonomies', 'gravityformsformstocpt' ),
	 ...
	 * @uses   GF_Forms_To_CPT::settings_post_date_custom()
	 * @uses   GF_Forms_To_CPT::settings_post_date_field()
	 *
	...
	public function settings_post_date_field( $field, $field_value = '' ) {
		...
		return sprintf(
			'<div class="gform_gravityformsformstocpt_date_field"%s>%s%s</div>',
	...
	public function settings_post_date_custom( $field, $field_value = '' ) {
		...
		// Display form field options.
		return sprintf(
			'<div class="gform_gravityformsformstocpt_date_custom"%s>%s</div>',
	...
	public function settings_taxonomy_map( $field, $echo = true ) {
		...
		$key_field['choices']  = array(
			array(
				'label' => esc_html__( 'Select an Option', 'gravityformsformstocpt' ),
				'value' => '',
			),
			array(
				'label' => esc_html__( 'Field Value', 'gravityformsformstocpt' ),
				'value' => 'field',
			),
			array(
				'label' => sprintf( esc_html__( 'Assign %s', 'gravityformsformstocpt' ), $taxonomy->labels->singular_name ),
				'value' => 'term',
			),
		);
		...
		$value_field['choices'] = array(
			...
			array(
				'label' => sprintf( esc_html__( 'Add New %s', 'gravityformsformstocpt' ), $taxonomy->labels->singular_name ),
				'value' => 'gf_custom',
			),
		);
	...
	public function get_post_types_as_choices() {
		...
		if ( ! empty( $default_post_types ) ) {
			...
			$choices[] = array(
				'label' => esc_html__( 'WordPress Post Types', 'gravityformsformstocpt' ),
		 ...
		 */
		$args = apply_filters( 'gform_gravityformsformstocpt_args_pre_get_custom_post_types', $args );
		...
		if ( ! empty( $custom_post_types ) ) {
			...
			$choices[] = array(
				'label' => esc_html__( 'Custom Post Types', 'gravityformsformstocpt' ),
	...
	public function get_file_fields_as_choices( $single_file = false ) {
		 ...
		 */
		$choices = apply_filters( 'gform_gravityformsformstocpt_file_fields_choices', $choices, $form, $single_file );
	...
	public function get_post_meta_field_map() {
		...
		$meta_fields = array(
			array(
				'label' => esc_html__( 'Select a Custom Field Name', 'gravityformsformstocpt' ),
				'value' => '',
			),
		);
		...
		// Add custom option.
		$meta_fields[] = array(
			'label' => esc_html__( 'Add New Custom Field Name', 'gravityformsformstocpt' ),
			'value' => 'gf_custom',
		);
	...
	public function get_post_authors_as_choices() {
		...
		$choices = array(
			array(
				'label' => esc_html__( 'Logged In User', 'gravityformsformstocpt' ),
	...
	public function get_post_author_search_results() {
		...
		// Verify nonce.
		if ( false === wp_verify_nonce( rgget( 'nonce' ), 'gform_gravityformsformstocpt_author_search' ) ) {
			wp_send_json( array( 'message' => esc_html__( 'Access denied.', 'gravityformsformstocpt' ), 'results' => array() ) );
		}
		...
		$results = array(
			array(
				'id'   => 'logged-in-user',
				'text' => esc_html__( 'Logged In User', 'gravityformsformstocpt' ),
			),
		);
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			$fields[] = array(
				...
				'tooltip'       => sprintf(
					...
					esc_html__( 'Assign terms to the post using a field value, an existing term or by adding a new term.', 'gravityformsformstocpt' )
				),
	...
	public function get_taxonomy_map_search_results() {
		...
		// Verify nonce.
		if ( false === wp_verify_nonce( rgget( 'nonce' ), 'gform_gravityformsformstocpt_taxonomy_search' ) ) {
			wp_send_json( array( 'message' => esc_html__( 'Access denied.', 'gravityformsformstocpt' ), 'results' => array() ) );
		}
		...
		$results[] = array(
			'id'   => 'gf_custom',
			'text' => sprintf( esc_html__( 'Add New %s', 'gravityformsformstocpt' ), $taxonomy_object->labels->singular_name ),
		);
	...
	public function feed_list_columns() {
		...
		return array(
			'feedName'   => esc_html__( 'Name', 'gravityformsformstocpt' ),
			'postType'   => esc_html__( 'Post Type', 'gravityformsformstocpt' ),
			'postStatus' => esc_html__( 'Status', 'gravityformsformstocpt' ),
		);
	...
	public function get_column_value_postType( $feed = array() ) {
		...
		if ( ! rgars( $feed, 'meta/postType' ) ) {
			return esc_html__( 'Not Selected', 'gravityformsformstocpt' );
		}

		// Get post type object.
		$post_type = get_post_type_object( $feed['meta']['postType'] );

		return $post_type ? esc_html( $post_type->labels->singular_name ) : esc_html__( 'Not Available', 'gravityformsformstocpt' );
	...
	public function get_column_value_postStatus( $feed = array() ) {
		...
		return $post_status ? esc_html( $post_status->label ) : esc_html__( 'Not Available', 'gravityformsformstocpt' );
	 ...
	 * @uses   GF_Forms_To_CPT::create_post()
	 *
	 * @return array
	 */
	 ...
	 * @uses   GF_Forms_To_CPT::get_generic_map_fields()
	 * @uses   GF_Forms_To_CPT::get_mapped_taxonomies()
	 * @uses   GF_Forms_To_CPT::media_handle_upload()
	 *
	 * @return array
	 */
	public function create_post( $feed, $entry, $form ) {
		 ...
		 * @param array $form  The current Form object.
		 */
		$post = gf_apply_filters( array( 'gform_gravityformsformstocpt_post', $form['id'] ), $post, $feed, $entry, $form );
		 ...
		 * @param array $form     The current Form object.
		 */
		gf_do_action( array( 'gform_gravityformsformstocpt_post_after_creation', $form['id'] ), $post['ID'], $feed, $entry, $form );
	...
	public function get_mapped_taxonomies( $feed, $entry, $form ) {
		 ...
		 */
		$separator = apply_filters( 'gform_gravityformsformstocpt_term_separator', ',' );
	...
	/**
	 * Add Post Creation By Trigger posts meta box to the entry detail page.
	 *
	 * @since  1.0
	 * @access public
	 *
	...
	public function register_meta_box( $meta_boxes, $entry ) {
		...
		if ( $created_posts ) {
			$meta_boxes[ $this->_slug ] = array(
				'title'    => esc_html__( 'Created Posts', 'gravityformsformstocpt' ),
	...
	/**
	 * Display created posts in Post Creation By Trigger entry meta box.
	 *
	 * @since  1.0
	 * @access public
	 *
	...
	public function add_posts_meta_box( $args ) {
		...
		foreach ( $created_posts as $post_info ) {
			...
			printf(
				...
				esc_html__( 'View Post', 'gravityformsformstocpt' ),
				get_edit_post_link( $post_id ),
				esc_html__( 'Edit Post', 'gravityformsformstocpt' )
			);
	...
	/**
	 * Export Post Creation By Trigger Add-On feeds when exporting form.
	 *
	 * @since  1.0
	 * @access public
	 *
	...
	/**
	 * Import Post Creation By Trigger Add-On feeds when importing form.
	 *
	 * @since  1.0
	 * @access public
	 *
	...
	public function import_feeds_with_form( $forms ) {
		...
		foreach ( $forms as $import_form ) {
			...
			// If the form does not have any Post Creation By Trigger Add-On feeds, skip.
	...
	public function filter_gform_pre_replace_merge_tags( $text, $form, $entry, $url_encode = false, $esc_html = true, $nl2br = true, $format = 'html' ) {
		...
		// If Post Creation By Trigger Media merge tag was found, replace merge tag.
