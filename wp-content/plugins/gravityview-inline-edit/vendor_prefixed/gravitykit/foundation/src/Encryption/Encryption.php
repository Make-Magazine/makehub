...
class Encryption {
	...
	private function __construct( $secret_key = '' ) {
		...
		if ( strlen( $secret_key ) !== SODIUM_CRYPTO_SECRETBOX_KEYBYTES ) {
			throw new Exception(
				strtr(
					esc_html_x( 'Encryption key is not the correct size (must be [size] bytes).', 'Placeholders inside [] are not to be translated.', 'gk-gravityedit' ),
	...
	public function encrypt( $data, $use_random_nonce = true, $custom_nonce = null ) {
		...
		if ( strlen( $nonce ) !== SODIUM_CRYPTO_SECRETBOX_NONCEBYTES ) {
			...
			throw new Exception(
				strtr(
					esc_html_x( 'Encryption nonce is not the correct size (must be [size] bytes).', 'Placeholders inside [] are not to be translated.', 'gk-gravityedit' ),
