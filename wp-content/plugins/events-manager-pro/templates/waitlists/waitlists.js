...
document.addEventListener("em_booking_form_init", function( e ) {
	...
	booking_form.addEventListener('em_booking_success', function (e) {
		em_booking_success_cleanup( booking_form );
	});
});

function em_booking_success_cleanup( booking_form ){
    ...
    if( wrapper !== null && wrapper !== parent ){
        let others = wrapper.querySelectorAll(':scope > div:not(#'+ parent.getAttribute('id') +')');
        if( others !== null ){
