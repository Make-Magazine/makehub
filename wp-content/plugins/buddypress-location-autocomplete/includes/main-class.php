...
if ( ! class_exists( 'BuddyBoss_BPLA_Plugin' ) ):
	...
	/**
	 *
	 * BuddyBoss BPLA Main Plugin Controller
	 * *************************************
	 *
	 *
	 */
	class BuddyBoss_BPLA_Plugin {
		/* Includes
		 * ===================================================================
		 */
		...
		/**
		 * BuddyBoss BPLA uses many variables, most of which can be filtered to
		 ...
		 *
		 * @see BuddyBoss_BPLA_Plugin::setup_globals()
		 * @var array
		 */
		private $data;
		...
		/**
		 * Main BuddyBoss BPLA Instance.
		 *
		 * BuddyBoss BPLA is great
		 * Please load it only one time
		 * For this, we thank you
		 *
		 * Insures that only one instance of BuddyBoss BPLA exists in memory at any
		 * one time. Also prevents needing to define globals all over the place.
		 *
		 * @since BuddyBoss BPLA (1.0.0)
		 *
		 * @static object $instance
		 * @uses BuddyBoss_BPLA_Plugin::setup_globals() Setup the globals needed.
		 * @uses BuddyBoss_BPLA_Plugin::setup_actions() Setup the hooks and actions.
		 * @uses BuddyBoss_BPLA_Plugin::setup_textdomain() Setup the plugin's language file.
		 * @see bp_bpla()
		 *
		 * @return BuddyBoss BPLA The one true BuddyBoss.
		 */
		public static function instance() {
			...
			if ( null === $instance ) {
				$instance = new BuddyBoss_BPLA_Plugin();
		...
		/**
		 * A dummy constructor to prevent BuddyBoss BPLA from being loaded more than once.
		 *
		 * @since BuddyBoss BPLA (1.0.0)
		 * @see BuddyBoss_BPLA_Plugin::instance()
		 * @see buddypress()
		 */
		...
		/**
		 * A dummy magic method to prevent BuddyBoss BPLA from being cloned.
		 *
		 * @since BuddyBoss BPLA (1.0.0)
		 */
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, __( 'Cheatin&#8217; huh?', 'bp-location-autocomplete' ), '1.0.0' );
		}

		/**
		 * A dummy magic method to prevent BuddyBoss BPLA from being unserialized.
		 *
		 * @since BuddyBoss BPLA (1.0.0)
		 */
		...
		/**
		 * Magic method for getting BuddyBoss BPLA varibles.
		 *
		 * @since BuddyBoss BPLA (1.0.0)
		 */
		...
		/**
		 * Magic method for setting BuddyBoss BPLA varibles.
		 *
		 * @since BuddyBoss BPLA (1.0.0)
		 */
		...
		/**
		 * Magic method for unsetting BuddyBoss BPLA variables.
		 *
		 * @since BuddyBoss BPLA (1.0.0)
		 */
		...
		/**
		 * Setup BuddyBoss BPLA plugin global variables.
		 *
		 * @since BuddyBoss BPLA (1.0.0)
		 * @access private
		 *
		 * @uses plugin_dir_path() To generate BuddyBoss BPLA plugin path.
		 * @uses plugin_dir_url() To generate BuddyBoss BPLA plugin url.
		...
		private function setup_globals( $args = array() ) {
			...
			$saved_options = $this->network_activated ? get_site_option( 'bp_bpla_plugin_options' ) : get_option( 'bp_bpla_plugin_options' );
			...
			/** Versions ************************************************* */
			$this->version = BUDDYBOSS_BPLA_PLUGIN_VERSION;
			$this->db_version = BUDDYBOSS_BPLA_PLUGIN_DB_VERSION;

			/** Paths ***************************************************** */
			// BuddyBoss BPLA root directory
			$this->file = BUDDYBOSS_BPLA_PLUGIN_FILE;
			$this->basename = plugin_basename( $this->file );
			$this->plugin_dir = BUDDYBOSS_BPLA_PLUGIN_DIR;
			$this->plugin_url = BUDDYBOSS_BPLA_PLUGIN_URL;
		...
		/**
		 * Setup BuddyBoss BPLA main actions
		 *
		...
		public function setup_textdomain() {
			$domain = 'bp-location-autocomplete';
			$locale = apply_filters( 'plugin_locale', get_locale(), $domain );

			//first try to load from wp-content/languages/plugins/ directory
			load_textdomain( $domain, WP_LANG_DIR . '/plugins/' . $domain . '-' . $locale . '.mo' );

			//if not found, then load from buddypress-location-autocomplete/languages/ directory
			load_plugin_textdomain( 'buddypress-location-autocomplete', false, $this->lang_dir );
		}
		...
		public function load_admin() {
			...
			$this->admin = BuddyBoss_BPLA_Admin::instance();
		}
		 ...
		 * @access private
		 *
		 * @uses BuddyBoss_BPLA_Plugin::do_includes() Loads array of files in the include folder
		 */
		private function load_main() {
			$this->do_includes( $this->main_includes );

			$this->component = new BuddyBoss_BPLA_BP_Component();
		}
		 ...
		 * 
		 * @uses apply_filters() Filters option values with 'bp_bpla_option' &
		 *                       'bp_bpla_option_{$option_name}'
		...
		public function option( $key ) {
			...
			$option = apply_filters( 'bp_bpla_option', $option );

			// Option specific filter name is converted to lowercase
			$filter_name = sprintf( 'bp_bpla_option_%s', strtolower( $key ) );
			$option = apply_filters( $filter_name, $option );

			return $option;
		}

	} // End class BuddyBoss_BPLA_Plugin
