...
add_filter( 'bboss_licensed_packages', 'lacbp_register_licensed_package' );
function lacbp_register_licensed_package( $packages=array() ){
	$packages['bblacbp'] = array(
		'id'       => 'bblacbp',
		'name'     => __( 'Location Autocomplete', 'bp-location-autocomplete' ),
		'products' => array(
			'BBLACBP' => array(
...
add_filter( 'bboss_updatable_products', 'lacbp_register_updatable_product' );
function lacbp_register_updatable_product( $products ){
    //key should be exactly the same as product key above
    $products['LACBP'] = array(
        //'path'  => plugin_basename(__FILE__),
        //@todo: this needs to be updated for all products
        'path'      => 'buddypress-location-autocomplete/bp-location-autocomplete.php',
        'id'        => 466,
        'software_ids'  => array( 'LACBP_1S', 'LACBP_5S', 'LACBP_20S' ),
        'type'      => 'plugin',
    );
...
if( !function_exists( 'bboss_notice_install_updater' ) ) {
    ...
    function bboss_notice_install_updater(){
        ...
        $notice_install_buddyboss_updater = sprintf( __( "To receive updates for BuddyBoss products, please install and activate the plugin %s", 'bp-location-autocomplete' ), $plugin_link );
