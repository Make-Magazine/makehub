...
//!! universal-nav/js/login.js
// Start putting login related scripts here and maybe someday we can even move in auth0

jQuery(document).ready(function(){
	var pathname = window.location.pathname;
	if(!navigator.cookieEnabled && (beginsWith("/authenticate-redirect", pathname) || beginsWith("/authenticated", pathname))) {
...
;//!!
//!! universal-nav/js/newsletter.js
...
function newsSignup() {
  // e.preventDefault();
   ...
   if (dest == 'MC') {
      var bla = jQuery('#mce-EMAIL').val();
      if(!bla) {
         //jQuery('.error-message').removeClass('hidden');
         grecaptcha.reset();
         return;
      }
      jQuery.post(postURL, jQuery('#mc-embedded-subscribe-form').serialize());
      ...
      jQuery.post(postURL, jQuery('#wc-embedded-subscribe-form').serialize());
      jQuery("#wc-email").val("");
   }
...
jQuery(document).ready(function(){
   ...
   function individualNewsletter(header, text, slid, dest, groupID) {
      jQuery(".newsletter-section h3").text(header);
      jQuery(".newsletter-section .more-details").text(text);
      if (dest === 'MC') {
         jQuery('#dest').val("MC");
         jQuery('#wc-embedded-subscribe-form').hide();
         //add code to set group id here
         jQuery('#mc-embedded-subscribe-form').append('<input style="display:none" type="checkbox" value="1" name="group['+groupID+'][1]" id="mce-group['+groupID+']-'+groupID+'-0" checked>');
         jQuery("#postURL").val("https://makermedia.us9.list-manage.com/subscribe/post?u=4e536d5744e71c0af50c0678c&amp;id=" + slid);
      } else {
         jQuery("#postURL").val("https://secure.whatcounts.com/bin/listctrl");
         jQuery('#dest').val("WC");
         jQuery('#mc-embedded-subscribe-form').hide();
         jQuery(".whatcounts-signup1 .slid").val(slid);
         jQuery(".whatcounts-signup1 .slid").attr("id", "list_" + slid + "_yes");
         jQuery(".whatcounts-signup1 input[name='custom_source']").val(window.location.hostname + window.location.pathname);
         jQuery(".whatcounts-signup1 input[name='custom_host']").val(window.location.hostname);
      }
   }
   ...
   switch (site) {
      ...
      case "dev.makezine.com":
         jQuery(".makerfaire-checkboxes").remove();
         individualNewsletter("Sign up for the Make: magazine newsletter", "Weekly editors picks from the Make: blog", "6B5869DC547D3D46B52F3516A785F101", 'WC','');
			//if(php_vars.disp_news_belt_makezine == 1) { beltOff(); }
			beltOff();
         break;
      case "makeco":
      ...
      case "dev.make.co":
         jQuery(".makerfaire-checkboxes").remove();
         individualNewsletter("Sign up for the Make: Membership newsletter", "For Make: Members — thoughts on how makers earn, learn, and live, plus new platform updates and exclusive members-only content", "609e43360a", 'MC','38209');
			//if(php_vars.disp_news_belt_makeco == 1) { beltOff(); }
			beltOff();
         break;
      case "makershed":
		case "www.makershed":
         jQuery(".makerfaire-checkboxes").remove();
         individualNewsletter("Sign up for the Maker Shed newsletter", "Be the first to learn about new products, plus exclusive discounts", "6B5869DC547D3D46510F6AB3E701BA0A", 'WC','');
			beltOff();
			//if(php_vars.disp_news_belt_makershed == 1) { beltOff(); }
         break;
      case "maker-faire":
      case "makerfaire":
      ...
      case "dev.makerfaire.com":
         jQuery(".makerfaire-checkboxes").css("display", "block");
         individualNewsletter("Sign up for the Maker Faire newsletter", "Weekly updates about the biggest celebration of invention, creativity, and curiosity that inspires the future", "6B5869DC547D3D46E66DEF1987C64E7A", 'WC','');
			beltOff();
			//if(php_vars.disp_news_belt_makerfaire == 1) { beltOff(); }
         break;
      case "maker-media":
      case "makermedia":
      case "makermedia.wpengine.com":
      case "makermedia.staging.wpengine.com":
      case "makermedia.com":
      case "stage.makermedia.com":
      case "dev.makermedia.com":
         jQuery(".makerfaire-checkboxes").remove();
         individualNewsletter("Sign up for the Make: magazine newsletter", "Weekly editors picks from the Make: blog", "6B5869DC547D3D46B52F3516A785F101", 'WC','');
			beltOff();
			//if(php_vars.disp_news_belt_makermedia == 1) { beltOff(); }
         //jQuery("footer .newsletter-footer").remove();
         break;
		case "makerspaces":
		...
		case "dev.makerspace.com":
			jQuery(".makerfaire-checkboxes").remove();
         individualNewsletter("Stay in the loop with our newsletter", "Stay inspired and get fresh updates", "6B5869DC547D3D467B33E192ADD9BE4B", 'WC','');
			beltOff();
			break;
   ...
   switch (shareSection) { // these aren't really universal yet, but here for the future
      ...
      case "makershare.com/learning":
         jQuery(".makerfaire-checkboxes").remove();
         individualNewsletter("Sign up for the Maker Share newsletter", "Project highlights from our Maker Share community", "6B5869DC547D3D46072290AE725EC932", "makershare.com/learning", 'WC','');
			beltOff();
         break;
      case "maker-share/":
      case "makershare/":
      ...
      case "makershare.com/":
         jQuery(".makerfaire-checkboxes").remove();
         individualNewsletter("Sign up for the Maker Share newsletter", "Project highlights from our Maker Share community", "6B5869DC547D3D46072290AE725EC932", "makershare.com", 'WC','');
			beltOff();
         break;
      default:
         break;
...
//!! universal-nav/js/owl.carousel.js
/**
 * Owl Carousel v2.3.4
...
;//!!
//!! universal-nav/js/universal-nav.js
	...
	jQuery(window).bind("load", function() {
		...
		switch(shareSection) {
			...
			case "makershare.com/learning":
				...
				nextItemUnderNav = jQuery(".site__content");
				break;
			case "maker-share/":
			case "makershare/":
			case "makeshare.wpengine.com/":
